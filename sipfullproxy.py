...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 OK/Accepted")
                return
        else:
            now = int(time.time())
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Bad Request - Client Error")
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("480 Temporarily Unavailable - Try again later")
        else:
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse("406 Request Not Acceptable")
        else:
            self.sendResponse("500 Internal Server Error")
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            request_uri = self.data[0]  # .decode('Windows-1252')
